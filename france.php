...
<body>
    ...
    <div class="nav-logo-cont"><span class="nav-logo" id="nav-logo">france</span></div>
  ...
  function nav() {
	...
	function menu_open() {
  	...
  	function dataFeedPlace () {
  		...
  		request.open("get", "data/place-france.json");
  		request.onload = () => {
  			try {
      ...
      setTimeout(function(){window.location.href = ""+path+"/place.html";}, 2000);
      	...
      	document.cookie = 'country=france';
    ...
    // redirect(clicked_id);
  	...
  	function populatePlace(json) {
  		...
  		for (var i = 0; i < json.length; i++) {
  			...
  			document.getElementById("place-img-"+i).style.background = "url('img/france/"+json[i][1]+"/" + json[i][0] + "') no-repeat";
