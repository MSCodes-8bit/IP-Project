...
<body>
    ...
    <div class="nav-logo-cont"><span class="nav-logo" id="nav-logo">turkey</span></div>
...
<script type="text/javascript">
  ...
  //   menu_cont.className = "nav-menu-cont";
  // });
	...
	function menu_open() {
  	...
  	function dataFeedPlace () {
  		...
  		request.open("get", "data/place-turkey.json");
  		request.onload = () => {
  			try {
  	...
  	function page_animate(clicked_id) {
    ...
    // document.body.scrollTop = a;
      ...
      // var trigger = await window.location.href = ""+path+"/place.html";
      	...
      	document.cookie = 'country=turkey';
    ...
    // redirect(clicked_id);
  	...
  	function populatePlace(json) {
  		...
  		for (var i = 0; i < json.length; i++) {
  			placeDiv.innerHTML += '<button id="place-page-'+i+'" name="'+json[i][1]+'" class="minimal" onclick="redirect(this.id)"><div id="changePage-'+i+'" class="card"><img class="place" id="place-img-'+i+'"></img><div id="info-'+i+'" class="info"><div id="p-name" class="p-name"><span>'+json[i][1]+'</span></div><div id="desc" class="desc"><span>'+ json[i][2] +'</span></div></div></div></button>';
  			document.getElementById("place-img-"+i).style.background = "url('img/turkey/"+json[i][1]+"/" + json[i][0] + "') no-repeat";
      ...
      var wid = document.body.offsetWidth;
       if(wid<=414) {
        for (var j=0;j<json.length;j++){
          document.getElementById("place-img-"+j).style.backgroundAttachment = "intial";
        }
       }
       else {
        for (var j=0;j<json.length;j++){
